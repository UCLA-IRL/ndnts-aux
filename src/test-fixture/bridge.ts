...
class BridgeTransport extends Transport {
  ...
  constructor(bridgeName: string, relay: RelayFunc, private readonly closePromise: Promise<undefined>) {
...
function makeRelayFunc(relay: Relay): RelayFunc {
...
export function create({
  ...
  const faceA = fwA.addFace(new L3Face(tA, { advertiseFrom: false }) as any);
  L3Face.processAddRoutes(faceA, routesAB);
  // deno-lint-ignore no-explicit-any
  const faceB = fwB.addFace(new L3Face(tB, { advertiseFrom: false }) as any);
